...
| Development source (this revision) | Unreleased; not the stable download | Since 0.8.7, source adds four-step onboarding with practice dictation, new dictation controls, menu-bar Transforms, reading-view edits, task-based AI routing, Apple Intelligence, and an Orukeet preview. Experimental Voice Control, voice profiles, and encrypted share links remain gated off. See the [release/flag table](spec/README.md#release-channels-and-feature-flags) and [CLI changelog](Sources/CLI/CHANGELOG.md); check the source commit before attributing these changes to the stable DMG. |
