...
4. **Stop / switch ordering** — scheduler rejects an interactive `.dictation` job while a live session exists (`:426`). Preview holds **no** live-session reservation, but it still needs bounded cancellation: on stop, cancel preview → bounded drain → final; on engine switch, cancel preview → bounded drain → switch if drained, or fast `engineBusy` if runtime work is still active.
...
- **Stop / switch**: cancel preview → bounded drain. Final never queues behind a preview pass; engine switch proceeds when drained or fails fast with `engineBusy` rather than hanging or reloading under active inference.
