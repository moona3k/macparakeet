...
## Integration boundaries after the CI change
...
## Original audit verification boundary
...
The candidate descriptions above record the audited source snapshot. Follow-up implementation is tracked separately so proposed deletions are not confused with verified changes. The seven test/fix PRs (#1162–#1167 and #1169) are merged, as are the audit and evidence documentation in [#1168](https://github.com/moona3k/macparakeet/pull/1168) and [#1170](https://github.com/moona3k/macparakeet/pull/1170). Local fault/revert checks and hosted CI remain distinct evidence.
...
PRs #1162–#1167 and #1169 merged after their full hosted behavior/Swift 6 and distribution gates passed on the reviewed heads. Those gates are separate from the local fault/revert checks below and do not establish physical audio, native GUI, or real-model qualification.
...
Cancellation implementation in [#1169](https://github.com/moona3k/macparakeet/pull/1169) was locally verified at `f103d317e7e910a52d7220fd882dba3e489fa75f`: 168 focused tests passed, followed by 63 checks after strengthening coordinator-task settlement and 20 final focused checks after formatting cleanup. Held STT/formatter cases failed against the original service. Real temporary files verify cleanup with Save audio off, and preserved cancelled transcripts are asserted. The coordinator test joins a read-only snapshot of its owned processing task before checking the connected clipboard; service idle alone was insufficient. Independent correctness and maintainability reviewers found no remaining issue at that head.
...
The optional search-composition and single registration deletions remain unselected; neither offers meaningful demonstrated runtime savings. The two AEC characterization cases, privacy architecture guards, and useful deterministic race hooks remain in place. The expanded journeys below now provide executable qualification infrastructure and stronger process-level coverage; native GUI, real inference, and physical routes remain separate runtime evidence. Hosted runner queue delay must also be measured separately from occupied build/test time; a burst of follow-up PRs can wait even when their execution paths are shorter.

## Expanded end-to-end qualification

The cheap CLI round trip and cancellation/coordinator boundary above run in ordinary deterministic CI. The expanded journeys merged in PRs [#1171](https://github.com/moona3k/macparakeet/pull/1171), [#1172](https://github.com/moona3k/macparakeet/pull/1172), and [#1173](https://github.com/moona3k/macparakeet/pull/1173); their reviewed heads passed hosted CI. The [final PR-run table](2026-09-25-ci-cost-and-test-strategy.md#final-qualification-pr-runs) records timings; the [combined-main run](https://github.com/moona3k/macparakeet/actions/runs/36247644540) at `529e23ad` also passed in 27m 05s, including explicit recovery execution (one test, zero failures, 10.203 seconds). They separate controlled process tests from qualification that needs a logged-in account, accepted model assets, or actual devices.

| Journey | Implementation and checks | Remaining evidence |
| --- | --- | --- |
| Native Library edit/save/relaunch/export | [#1172](https://github.com/moona3k/macparakeet/pull/1172): real Dev app plus small Accessibility runner; production migrations/artifact store seed owned synthetic content; the runner asserts notes survive ordinary app quit/relaunch and checks separate note artifacts and transcript export. Nine native helper tests passed; the runner observes durable notes through a read-only SQLite check and cleans up owned launches. | Actual GUI journey not run. Requires the dedicated logged-in account, completed onboarding, and Accessibility permission. Normal startup may warm/download model assets; keep microphone access denied for this Library-only flow. No offline or capture claim. |
| Writer termination and fresh-process recovery | [#1173](https://github.com/moona3k/macparakeet/pull/1173): production recording service writes synthetic audio and its real lock; an owned child receives SIGKILL; fresh processes recover/materialize once and reject stale retry. Three recorded post-fix local runs took 7.174, 7.077, and 7.578 seconds, excluding build time. The exact opt-in test is routed into the required behavior job with its own log. | The dedicated hosted step passed: one test, zero failures, 9.541 seconds in [36221389691](https://github.com/moona3k/macparakeet/actions/runs/36221389691). Synthetic capture and stub STT do not qualify devices, power loss, or recognition accuracy. |
| Real CLI/model/file qualification | [#1171](https://github.com/moona3k/macparakeet/pull/1171): prebuilt CLI, accepted model-file hashes, fresh history read, engine attribution, known speech words, and Markdown content; network-denied child commands and owned-process timeouts. After review, 18 model-specific helper tests passed, including descendant readiness/PID-handshake checks and cleanup negative controls. Disabling process-group cleanup left detected survivors; fixture cleanup then stopped them. Real speech synthesis/conversion plus a fake CLI checked orchestration and wrong-export rejection. | Real model inference not run. Requires the disposable account, accepted Parakeet v3 assets, and intended CLI. The fake CLI control is evidence about the driver only. |
| Physical dictation and meeting routes | Explicit procedures cover delivery, cancel/restart, permission denial, dual-source recording, Bluetooth transition, and missing/late sources; each requires observable results and build/model/device/permission evidence. | All physical rows not run. These procedures are manual qualification, not automated coverage or a device-quality result. |

The recovery implementation exposed a real disagreement: the saved row said it was recovered while the manifest did not. Recovery now refreshes canonical artifacts before settling its lock; a failed refresh retains the lock and retries without repeated recognition, preserving prompt results. Independent review then found that retained lock notes could resurrect deliberately cleared canonical notes. A real-database failure/clear/retry test reproduced that defect; the completed row now wins, including nil/empty notes. All 49 focused recovery tests passed after the final fixture correction, with 20 artifact-store tests passing in the earlier focused group. Independent artifact refreshes across services can still race from different snapshots; this work does not establish global serialization of derived files.

The native runner checks persisted notes through a read-only SQLite query, then requests normal AppKit quit so derived meeting artifacts can finish. It does not rely on the transient visible Saved indicator. Transcript export and meeting notes also have distinct contracts: UI transcript export excludes notes, so both outputs are checked separately. Failure evidence currently includes logs and result JSON, not screenshots or a failure-bound commit hash; record the tested checkout externally when qualifying. The native import-with-STT and delayed-save/navigation UI variants remain later extensions, with their deterministic race coverage retained.

The first hosted recovery run [36218994386](https://github.com/moona3k/macparakeet/actions/runs/36218994386) failed because the real journey lacked FFmpeg; one unit fixture also unintentionally invoked real FFmpeg. The final head installs FFmpeg in CI and uses the existing mock in that unit fixture. Only the final dedicated execution above establishes hosted process-recovery proof; see the [recovery guide](../testing/meeting-process-recovery.md#hosted-verification).

No physical test result is inferred from these automated checks. Missing prerequisites fail the opt-in runners instead of producing a pass. The [CI cost report](2026-09-25-ci-cost-and-test-strategy.md#implemented-result) contains the measured pipeline before/after comparison; adding these journeys is about fault detection, not a claimed reduction in test execution time.
