...
- R6. `createdAt` records the chosen historical meeting date. A separate optional `audioRetentionStartedAt` records when the managed audio entered MacParakeet; retention uses it when present and falls back to `createdAt` for existing rows.
