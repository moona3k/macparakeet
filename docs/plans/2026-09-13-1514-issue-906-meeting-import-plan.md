...
- R6. `createdAt` records the chosen historical meeting date. A separate optional `audioRetentionStartedAt` records when the managed audio entered MacParakeet; retention uses it when present and falls back to `createdAt` for existing rows. Keep-forever and timed retention apply normally. Delete-immediately removes managed audio after successful processing but preserves it while a published import still needs Retry.
