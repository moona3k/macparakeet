...
`DictationFlowCoordinator` gains two observer hooks: `onFlowStateChanged` (drives the lit cap inside the box and the "Listening" / "Transcribing" line) and `onDictationDelivered(text)` (fires after a successful insert or copy). The view model records the transcript as the practice result. If the box still does not contain that text after a short grace period (a slow or failed paste, or a clipboard-only trigger), the view model appends it. This covers the normal paste race without delaying Continue. If a later dictation repeats text already in the box and its paste misses the box, the fallback cannot distinguish that from a successful paste; the delivered dictation still remains in history.
