...
| LOGTEL-004 | P2 | FIXED | Add safe STT dimensions to operation events | `dictation_operation` and `transcription_operation` now emit `speech_engine` and `engine_variant` from authoritative `STTResult` attribution/persisted operation output, not current settings. |
| LOGTEL-005 | P2 | TODO | Implement explicit diagnostic export bundle | User-triggered only. Include recent MacParakeet `os.Logger` entries, local audio diagnostics, app/build info, and redacted runtime metadata. No audio, transcripts, notes, prompts, file names, paths, URLs, API keys, or device UIDs. |
| LOGTEL-006 | P2 | IN PROGRESS | Normalize new local OSLog lines | Targeted dictation/transcription lifecycle logs were normalized in this follow-up, including making a captured audio path private. Continue using stable event-style messages with `key=value`, safe dimensions, classified `error_type`, and `.private` for sensitive values when touching nearby code. Avoid repo-wide churn-only rewrites. |
| LOGTEL-007 | P2 | TODO | Resolve `model_download_cancelled` drift | Either implement the Swift event or remove the stale Worker allowlist/docs reference. |
