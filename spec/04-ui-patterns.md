...
- **Transcript pane** — scrolling live preview grouped into reading paragraphs, with one source label and timestamp per paragraph ([Me] = mic, [Them] = system audio); lag notice appears when preview chunks fall behind or are dropped. While listening, the empty state uses the slowly rotating seed-of-life. When live transcription is off, that mark sits still at rest pose with faded coral (no rotation or breathing pulse) so it does not read as "listening"; pause remains a full-color freeze of the living animation. The Notes pane keeps the living rosette because the meeting is still recording.
...
- When meeting preview and final routes differ, the recording panel attributes both. When preview is disabled by the user or unsupported by the live engine, the panel says "Live transcription is off" and confirms audio is recording for transcription after stop. In that Transcript empty state the seed-of-life sits still and faded rather than spinning. Settings explains that the live transcription toggle applies to the next recording.
