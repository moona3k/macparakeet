...
> Status: **IMPLEMENTED** — Phases 1 (notify-only) and 2 (auto-start + countdown + auto-stop) are implemented and enabled (`AppFeatures.calendarEnabled = true` as of the post-#318 reliability hardening: generation-bound auto-stop, mid-flight teardown, RSVP/zero-duration guards, reschedule re-fire, forgiveness-window polling). Auto-start defaults to mode `.off`, so users opt in via onboarding or Settings — nothing changes for existing users until they do. Phase 3 (late-join, retro-link, generic URL extraction) remains **PROPOSED** (see Phased Rollout below).
> Date: 2026-04-19
