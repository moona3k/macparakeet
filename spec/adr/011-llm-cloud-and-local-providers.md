...
| Local Formatting Model | CLI | N/A (subprocess) | N/A (no auth; user-installed deps) |
...
**Amendment (2026-04-30): Local Formatting Model provider.** Users who want fully-local cleanup without running a long-lived Ollama or LM Studio process can opt into the bundled `macparakeet-cleanup` Python CLI. The CLI itself is small (~76 KB Python module + launcher); the heavy parts — `mlx`, `mlx-lm`, `huggingface_hub`, and the MLX model weights — are **not bundled**. They are installed on demand from Settings into `~/Library/Application Support/MacParakeet/cleanup-runtime/` (Python deps, ~350 MB) and `~/Library/Application Support/MacParakeet/models/llm/` (MLX model snapshots, ~2 GB depending on variant). The system `/usr/bin/python3` is used; we do not ship a Python interpreter. This preserves Locked Decision #1 (no bundled LLM runtime, zero out-of-box GPU/memory impact, lightweight distribution) while offering a turn-key local-LLM path for users who don't want to manage Ollama. Prompts are delivered via stdin + `--prompt-file`; the Swift `LocalFormattingModelClient` routes through `RoutingLLMClient.localFormattingModel`. The provider can be empty (rules-only cleanup, no MLX model required), `llm` (MLX), or `auto` (rules first, MLX on long/complex inputs). When the MLX runtime or model are missing, the UI gates Save behind explicit Install / Download buttons; in that state, requests fall back to the deterministic rules-only pipeline (with the missing-runtime error surfaced via `LocalFormattingModelDownloader.RuntimeStatus`) rather than silently producing empty output. Pure rules-only mode requires neither the MLX runtime nor a model download.
