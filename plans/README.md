...
| [2026-06-13-live-dictation-streaming-parakeet-and-preview-ui](active/2026-06-13-live-dictation-streaming-parakeet-and-preview-ui.md) | Live dictation preview — pill UI revamp + unified all-engine streaming | **TODO** | P2 | New (2026-06-13). **Part A** lifts the live preview out of the capsule so the pill keeps its shape (view-only, LOW risk, ship-alone). **Part B** extends the preview from Nemotron-only to **all three engines** via one generic **display-only re-transcription** source (LocalAgreement-2 over each engine's existing `[Float]` batch transcribe), behind `AppFeatures.liveDictationStreamingEnabled`. Key decision: **preview is display-only, never the paste** → engine-agnostic, no correctness surface, Whisper included. Final/paste path unchanged (Parakeet batch ~instant; Nemotron keeps native streaming). Rejected per-engine vendor streaming (sliding-window/AudioStreamTranscriber) for one generic mechanism. Amends REQ-STT-002. Design doc: `docs/research/live-dictation-streaming.md`. |
