...
| [2026-06-28-transcript-detail-refresh](active/2026-06-28-transcript-detail-refresh.md) | Transcript Detail Refresh (media-document reading) | **PARTIAL** (Phase 1 in branch) | P2 | The audit's "Move 1" (highest daily-use value). Mapping found much already ships — persistent `AudioScrubberBar`, auto-scroll-to-active, tap-to-seek, active-line highlight, selection — so scope narrows to the real gaps. **Phase 1 implemented on `feat/transcript-detail-refresh`:** U1 per-segment hover actions (play-from-here / copy / copy-with-timestamp via shared `TranscriptSegmentRow`) + U4 reading font-size control (persisted `transcriptFontScale`, applied to Text + Timed). Remaining: U2 in-transcript find (`⌘F`, testable `TranscriptFindModel`), U3 unify one quiet playback rail across video mode, U5 finish `TranscriptResultView` decomposition. Hard constraint: `TranscriptSegment` has no `endMs` (start-only). Grounded in Descript/IINA/iOS-Podcasts references. |
