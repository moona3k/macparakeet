...
|------|-------|--------|----------|-------------|
| [2026-09-13-fluidaudio-0.15.7](active/2026-09-13-fluidaudio-0.15.7.md) | FluidAudio 0.15.7 — exact speaker counts are not honoured (#1023) | **TODO — OWNER GATE (audio corpus)** | **P1** | Upstream FluidAudio #891: the speaker-count constraint was checked against the clusters that won an argmax while centroid construction kept every cluster with `pi > 1e-7`, so a cap looked compliant and constrained assignment revived the excluded cluster. `--speaker-count 1` on a 1:1 recording returns 2 speakers, and the same gate lets `MeetingSpeakerPrior`'s `max = n + 1` ceiling be exceeded. Fixed in 0.15.7; the app pins 0.15.6. Verified: no compile break (no call site touches the changed ASR enums or `melChunkContext`), but real behavioural exposure via `CustomVocabularyBoosting` (#866/#898/#900 rewrote that CTC surface), v3 no-mel long-form (#869) and the streaming seam (#895/#903), so ADR-010's STT regression pass is required alongside the diarization before/after. Bumping `pipelineRevision` re-keys `aggregationProfileId`; stored exemplars match at the tightened threshold by design and voice profiles stay off. Needs an audio corpus (1:1, three-voice, single-speaker) before execution. |
