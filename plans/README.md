...
| [2026-09-16-telemetry-observability-followthrough](active/2026-09-16-telemetry-observability-followthrough.md) | Dictation e2e timings, cheap activation SQL, pipeline health, LLM formatter labels | **PR OPEN** | P1 | Website allowlist, ingest remap, windowed activation SQL, and snapshot health probe are live on `/api/stats`. App `dictation_insert` + formatter `feature=formatter` still need merge. Do not deploy retention. Operator: one Cloudflare Worker-exception notification on `macparakeet-telemetry-snapshots`. |
