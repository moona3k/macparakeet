...
- An unset prompt preserves historical request parameters, including the
  current `0.7` temperature and native Ollama's `think: false`. The corrected
  Anthropic input budget reserves its inherited 4096 output tokens on both
  initial generation and regeneration, so near-limit input truncation can change.
...
- Unsupported fields are omitted, never translated. Invalid numeric values
  are rejected at decoding, repository and execution boundaries, not omitted.
...
- CLI inference-setting configuration flags and Transform settings; the CLI
  preserves/displays saved result-prompt settings and applies them on run.
- Persisted requested-settings/omission metadata and provider/model snapshots.
  Queue items capture requested settings; saved results retain effective-only
  receipts and resolve current provider/model configuration at execution.
...
adapter emits exactly one terminal event after its completion policy passes.
Ollama permits clean EOF after content with observed metadata, but both stream
paths reject provider error envelopes before decoding content responses.
Errors and cancellation never emit a successful terminal event.
