...
public protocol ShareManaging: Sendable {
    ...
    func updateContent(shareId: UUID, bundle: ShareBundle, projectionManifest: Data?, contentDigest: String?)
    ...
    func recoverOwnership(recoveryToken: ShareRecoveryToken, replacementRecoveryVerifier: String?) async throws
        -> ShareOwnerMetadata
...
public final class ShareManagementViewModel {
    ...
    public func prepareDraft(for share: SharePublication, updating: Bool) async {
        ...
        defer { isBusy = false }
        do {
            guard let source = try await sourceLoader(sourceID) else {
    ...
    public func changeExpiry(_ share: SharePublication, to date: Date) async {
        guard let service else { return }
        guard date > Date(), date <= share.maxExpiresAt,
            date.timeIntervalSince1970.rounded(.down) == date.timeIntervalSince1970
        else {
    ...
    public func canUpdate(_ share: SharePublication) -> Bool {
        canManage(share) && share.isContentUpdateEligibleLocally && links[share.id] != nil
            && availableSourceIDs.contains(share.id)
    }
    ...
    public func status(for share: SharePublication) -> String {
        ...
        if share.isTerminal {
            return
                "\(share.accessState == .expired ? "Expired" : "Stopped permanently"); encrypted copy deletion pending"
        }
        ...
        return share.contentWritable && links[share.id] != nil
            ? "Active snapshot" : "Active — management only on this Mac"
    }
    ...
    public func importRecovery(code: String) async {
        ...
        await perform {
            ...
            self.recoveryMessage =
                "Management restored. Earlier links are management-only; new sharing works normally. Set up and save a replacement recovery code."
            _ = try await service.refreshPublications()
        }
    }
    ...
    public func acknowledgeRecoverySaved() async {
        guard let service else { return }
        await perform {
            try await service.acknowledgeRecoveryCodeSaved(); self.recoveryCode = nil
        }
    }
    ...
    private func loadLocal() async throws {
        ...
        for share in publications {
            ...
            if !operations.contains(where: { $0.kind == .delete }),
                let link = try await service.confirmedLink(shareId: share.id)
            {
                nextLinks[share.id] = link
            }
            ...
            if let sourceID = share.transcriptionId, let sourceLoader, let source = try? await sourceLoader(sourceID) {
                ...
                if let manifest = share.projectionManifest.flatMap({
                    try? JSONDecoder().decode(ShareProjectionManifest.self, from: $0)
                }), let digest = share.contentDigest {
                    let current = await Task.detached(priority: .utility) {
                        try? source.bundle(manifest: manifest, publishedAt: share.updatedAt).contentDigest()
                    }.value
    ...
    public static func message(for error: Error) -> String {
        if let error = error as? ShareCoordinatorError {
            ...
            case .deviceCredentialSuperseded:
                return
                    "Another installation recovered this owner. This Mac can no longer manage these links. Use your current recovery code to restore management."
            case .recoverySwitchBlocked, .sharesNotTerminal:
                return
                    "Stop every existing link and wait for encrypted-copy deletion before switching owners. Your current credentials have been kept."
            case .operationInProgress: return "Another sharing operation is finishing. Please try again shortly."
            case .recoveryPending:
                return
                    "Recovery confirmation is pending. Refresh to reconcile it before making another recovery change."
            case .contentKeyUnavailable, .contentUpdateNotEligible:
                return
                    "This page is management-only on this Mac. You can change its expiration or stop it, but cannot recover its text or complete link."
        ...
        if error is ShareCredentialTokenError {
            return "That recovery code is not valid. Paste the complete code, starting with mpr1."
        }
        if let client = error as? ShareClientError, case .api(let api) = client {
            switch api.code {
            case .versionConflict:
                return
                    "The remote state changed. Refresh before trying again. An existing recovery code can only be replaced with proof of that code."
            case .quotaExceeded, .rateLimited:
                return "The sharing limit has been reached. Wait before trying again, or stop unused links."
            case .serviceUnavailable:
                return "New publication is temporarily unavailable. Existing links can still be stopped."
            case .unauthorized:
                return
                    "The credential or recovery proof was not accepted. Your existing local management state has been preserved."
            default: break
            }
        }
        return
            "The service has not confirmed this change. Your draft and any queued work are kept on this Mac. Refresh Shared pages to reconcile before trying again."
    }
}
