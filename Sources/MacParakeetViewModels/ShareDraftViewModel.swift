...
public struct ShareProjectionManifest: Codable, Equatable, Sendable {
    ...
    public init(summaryIDs: [UUID] = [], includeNotes: Bool = false, includeTranscript: Bool = false,
...
public struct ShareDraftSource: Sendable {
    ...
    public func bundle(manifest: ShareProjectionManifest, publishedAt: Date) throws -> ShareBundle {
        try ShareProjection.project(
            ...
            summaries: summaries.filter { manifest.summaryIDs.contains($0.id) }.map { ShareSummary(title: $0.title, markdown: $0.markdown) },
            selection: ShareSelection(includeSummary: !manifest.summaryIDs.isEmpty,
                                      includeNotes: manifest.includeNotes, includeTranscript: manifest.includeTranscript,
                                      transcriptOptions: TranscriptExportOptions(includeTimestamps: manifest.includeTimestamps,
...
public enum SharePresentationCopy {
    public static let disclosure = "Only the text in this preview is encrypted and uploaded. Audio is never included. Anyone with the complete link can read, copy and forward it. The hosted viewer and recipient’s browser handle the decryption key."
    public static let stopConfirmation = "Stop this link permanently? It cannot be turned back on. This cannot erase copies people have already saved. If you are offline, the link may still work until the service confirms the stop."
    public static let recovery = "A saved recovery code restores management, not the text or complete links. Without it, losing this Mac’s sharing credentials means you cannot stop a link early. Every link still expires."
}
...
public final class ShareDraftViewModel: Identifiable {
    ...
    public init(source: ShareDraftSource, service: any ShareManaging, updating: SharePublication? = nil,
        ...
        if let data = updating?.projectionManifest, let saved = try? JSONDecoder().decode(ShareProjectionManifest.self, from: data) {
            manifest = saved
        } else if let selectedSummaryID {
            manifest = ShareProjectionManifest(summaryIDs: [selectedSummaryID])
        } else {
            let summaries = source.summaries.filter { !$0.markdown.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
            let hasNotes = !(source.transcription.userNotes ?? "").trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            let meetingLike = source.transcription.sourceType == .meeting || !summaries.isEmpty || hasNotes
            manifest = ShareProjectionManifest(summaryIDs: summaries.map(\.id), includeNotes: hasNotes, includeTranscript: !meetingLike)
        }
    }

    public var canIncludeTimestamps: Bool { source.transcription.hasWordTimestamps && !source.transcription.isTranscriptEdited }
    public var canIncludeSpeakerLabels: Bool { source.transcription.hasSpeakerLabeledWords && !source.transcription.isTranscriptEdited }
    public var maximumExpiration: Date { updating?.maxExpiresAt ?? now().addingTimeInterval(7_776_000) }
    public var canPublish: Bool { preview != nil && preparedManifest == manifest && !isPreparing && !isPublishing && publication == nil }
    ...
    public func publish() async {
        ...
        let instant = now()
        guard expiresAt.timeIntervalSince1970.isFinite, expiresAt.timeIntervalSince1970.rounded(.down) == expiresAt.timeIntervalSince1970,
              expiresAt > instant, expiresAt <= maximumExpiration else {
        ...
        do {
            ...
            let digest = try bundle.contentDigest()
            if let updating {
                let result = try await service.updateContent(shareId: updating.id, bundle: bundle, projectionManifest: manifestData, contentDigest: digest)
                publication = result
                if result.contentRevision > updating.contentRevision { link = try await service.confirmedLink(shareId: result.id) }
            } else {
                let result = try await service.publish(bundle: bundle, transcriptionId: source.transcription.id,
            ...
            if let updating {
                if let operations = try? await service.pendingOperations(shareId: updating.id), operations.contains(where: { $0.kind == .contentUpdate }) { publication = updating }
            } else if let rows = try? await service.listPublications() {
                publication = rows.first { !existingIDs.contains($0.id) && $0.transcriptionId == source.transcription.id && $0.createdAt >= publishedAt.addingTimeInterval(-1) }
            }
        }
    }
}
