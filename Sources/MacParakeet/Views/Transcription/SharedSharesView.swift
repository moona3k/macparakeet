...
struct SharedSharesView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $changingExpiry) { share in
            VStack(alignment: .leading, spacing: 20) {
                ...
                Text(
                    "Original lifetime limit: \(share.maxExpiresAt.formatted(date: .abbreviated, time: .standard)). Stopped or expired links cannot be extended."
                )
                .foregroundStyle(.secondary)
                if let error = model.errorMessage { Text(error).foregroundStyle(DesignSystem.Colors.errorRed) }
                HStack {
        ...
        .confirmationDialog(
            ...
            Text(SharePresentationCopy.stopConfirmation)
        }
    }
    ...
    private func row(_ share: SharePublication) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(
                "Expires \(share.expiresAt.formatted(date: .abbreviated, time: .standard)) · Revision \(share.contentRevision)"
            )
            .font(.caption).foregroundStyle(.secondary)
            if share.isDetached {
                Text("The local source was removed. This record keeps its remote stop manageable.").font(.caption)
            }
            if let link = model.links[share.id] { ShareLinkActions(link: link) }
            HStack {
                if model.canUpdate(share) {
                    Button("Update shared page…") { Task { await model.prepareDraft(for: share, updating: true) } }
                        .parakeetAction(.secondary)
                }
                if model.canManage(share) {
                    Button("Expiration…") {
                        expiration = share.expiresAt; changingExpiry = share
                    }.parakeetAction(.secondary)
                }
                if !share.isTerminal && !model.credentialSuperseded
                    && !model.pending[share.id, default: []].contains(.delete)
                {
                    Button("Stop sharing…", role: .destructive) { stopping = share }.parakeetAction(.destructive)
                        .disabled(model.isBusy)
                }
                if share.isTerminal && model.availableSourceIDs.contains(share.id) {
                    Button("Create a new link…") { Task { await model.prepareDraft(for: share, updating: false) } }
                        .parakeetAction(.secondary).disabled(model.isBusy)
                }
                if share.deletionState == .complete && model.pending[share.id, default: []].isEmpty {
                    Button("Remove record from this Mac") { Task { await model.forget(share) } }.parakeetAction(
                        .secondary
                    ).disabled(model.isBusy)
                }
            }
...
struct ShareRecoveryView: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                if let code = model.recoveryCode {
                    ...
                    HStack {
                        ...
                        Button("I saved it") { Task { await model.acknowledgeRecoverySaved() } }.parakeetAction(
                            .primary
                        ).disabled(model.isBusy)
                    }
                } else {
                    Button("Set up a recovery code") { Task { await model.setUpRecovery() } }.parakeetAction(.primary)
                        .disabled(model.isBusy)
                    Text("Already have a saved code? Keep it. To replace or remove it, enter that code below.").font(
                        .caption
                    ).foregroundStyle(.secondary)
                }
                Divider()
                DisclosureGroup("Replace or remove my current code") {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack {
                            ...
                            Button("Remove recovery…", role: .destructive) { confirmRemoval = true }.parakeetAction(
                                .destructive)
                ...
                DisclosureGroup("Restore management from a recovery code") {
                    VStack(alignment: .leading, spacing: 12) {
                        Text(
                            "Restoring invalidates the previous installation’s management credential and the imported code. Earlier links become management-only. If this Mac manages a different owner, all its links must be stopped and deletion completed first."
                        )
                        .font(.callout).foregroundStyle(.secondary)
                        SecureField("Recovery code to restore", text: $importedCode)
                        Button("Restore management…") { confirmImport = true }.parakeetAction(.secondary).disabled(
                            model.isBusy || importedCode.isEmpty)
                    }.padding(.top, 10)
                }
                DisclosureGroup("I lost my configured recovery code") {
                    VStack(alignment: .leading, spacing: 12) {
                        Text(
                            "You can keep managing links from this Mac. Recovery cannot be reset without its code. Stop all outstanding links and wait for deletion; then you can start with a new anonymous owner."
                        )
                        .font(.callout).foregroundStyle(.secondary)
                        Button("Start fresh after all links are deleted…") { confirmFresh = true }.parakeetAction(
                            .secondary
                        ).disabled(model.isBusy)
                    }.padding(.top, 10)
                }
                ...
                HStack {
                    if model.isBusy { ProgressView().controlSize(.small) }
                    Button("Reconcile pending confirmation") { Task { await model.refresh() } }.parakeetAction(
                        .secondary
                    ).disabled(model.isBusy)
                }
            }
        ...
        .confirmationDialog("Remove recovery?", isPresented: $confirmRemoval) {
            Button("Remove recovery", role: .destructive) {
                Task {
                    await model.removeRecovery(proof: proof); if model.errorMessage == nil { proof = "" }
                }
            }
        } message: {
            Text("If this Mac loses its sharing credentials, you will no longer be able to stop existing links early.")
        }
        .confirmationDialog("Restore management on this Mac?", isPresented: $confirmImport) {
            Button("Restore management") {
                Task {
                    await model.importRecovery(code: importedCode); if model.errorMessage == nil { importedCode = "" }
                }
            }
        } message: {
            Text(
                "The imported code and previous installation’s management credential will stop working. Existing local authority is preserved if recovery fails."
            )
        }
        .confirmationDialog("Start a new anonymous owner?", isPresented: $confirmFresh) {
            ...
            Text(
                "This checks the service before discarding the old credential. It refuses until every old link is terminal and encrypted-copy deletion is complete."
            )
        }
    }
}
