...
public final class PromptLabelPolicyRepository: PromptLabelPolicyRepositoryProtocol, @unchecked Sendable {
    ...
    public func replaceTargetLabels(promptId: UUID, labelIds: Set<UUID>) throws {
        try dbQueue.write { db in
            try Self.replaceTargetLabels(promptId: promptId, labelIds: labelIds, in: db)
        }
    }

    /// Used by PromptEditingService so routing and prompt/version changes
    /// commit together. This helper never opens a separate transaction.
    static func replaceTargetLabels(promptId: UUID, labelIds: Set<UUID>, in db: Database) throws {
